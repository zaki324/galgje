...
$woordenlijst=$row= mysqli_fetch_assoc($result);
...
{
    $lengte=strlen($_SESSION['gokwoord']);
    $x=0;
...
if(isset($_POST["letter"]) and ($_POST["letter"]!=NULL))
{
    ...
    $offset=0;
    $positie = 0;
    $positie = strpos($_SESSION['gokwoord'],$letter,$offset);
    ...
    while($positie!== false)
    {
         ...
         {
              if(strpos($_SESSION['gokwoord'],$letter,0)=='0')
              {
    ...
    if((strpos($_SESSION['gokwoord'],$letter)===false) and (isset($letter)))
    {
...
if($poging==7)
{
    echo "<font color=red>Je hebt verloren! het woord was: </font><b>".$_SESSION['gokwoord']."</b>";
    exit;
}
...
echo '</div>';
echo "<br><br><br>".$_SESSION['gokwoord'];
